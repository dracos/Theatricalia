...
for n in range(1, 67):
	...
	for play_row in rows:
		...
		details = open('details/%s' % data['id']).read()
		...
		production, created = Production.objects.get_or_create(play=play, start_date=data['first'], end_date=data['last'])
